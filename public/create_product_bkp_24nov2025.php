...
if (isset($_POST['vendor']) && $_POST['vendor'] === 'ross') {
    ...
    $targetProductNumber = '010177';
        ...
        foreach ($jsonData['result'] as $index => $product) {
            ...
            $name = is_string($product['productName']['en'] ?? '') ? trim($product['productName']['en']) : '';
            $description = is_string($product['productText']['en'] ?? '') ? trim($product['productText']['en']) : '';
            $brandName = is_string($product['productBrand'] ?? '') ? trim($product['productBrand']) : '';
            $categoryName = is_string($product['productCategory'][0]['translation']['en'] ?? '') ? trim($product['productCategory'][0]['translation']['en']) : '';
            $imageUrlsParent = [];
            ...
            foreach ($colorsFound as $colorCode => $cdata) {
                $colorProperyData[] = [
                    'id' => Uuid::randomHex(),
                    'name' => $cdata['name'],
                    // you may want to store code => to map later, we'll attach code to option via array index
                    'code' => $colorCode
                ];
            }
            // size property options
        ...
        if (!empty($productCsvData)) {
            ...
            createProductCsv($productCsvData, $container);
            echo "🎉 Imported parent + " . (count($productCsvData)-1) . " variants successfully for product {$targetProductNumber}!";
        } else {
            echo "⚠️ Product {$targetProductNumber} not found or no variants available.";
        }
