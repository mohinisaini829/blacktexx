...
const { Component, Data, Utils, Classes, ExtensionAPI, Context } =
    Shopware;
const { Criteria } = Data;
...
Component.extend('sas-blog-detail', 'sw-cms-detail', {
    ...
    computed: {
        ...
        loadBlogCriteria() {
            ...
            criteria
                ...
                .addAssociation('assignedProducts.options.group')
                .addAssociation('tags')
    ...
    watch: {
        'blog.title': {
            handler(value) {
                ...
                if (value !== this.originalTile) {
                    this.onBlogTitleChanged(value);

                    return;
                }

                this.blog.slug = this.originalSlug;
            },
            immediate: false,
        },
    },
    ...
    methods: {
        async createdComponent() {
            ...
            if (this.cmsPageState?.setIsSystemDefaultLanguage) {
        ...
        loadBlog(blogId) {
            ...
            return this.blogRepository
                ...
                .then((entity) => {
                    ...
                    this.originalTile = entity.title;
        ...
        async addAdditionalSection(type, index) {
            const isSlugDuplicated = await this.duplicatedSlugValidation();
            if (isSlugDuplicated || !this.blogIsValid()) {
                this.createNotificationError({
                    message: this.$tc(
                        'sas-blog.detail.notification.error.pageInvalid',
                    ),
                });
        ...
        onSavePageEntity() {
            ...
            return this.pageRepository
                .save(this.page, Context.api, false)
                .then(() => {
                    this.isLoading = false;
        ...
        async duplicatedSlugValidation() {
            if (!this.localeLanguage) {
                return false;
            }

            this.isSlugDuplicated = false;

            const criteria = new Criteria();
            criteria.addFilter(Criteria.equals('slug', this.blog?.slug));
            criteria.addFilter(
                Criteria.not('AND', [Criteria.equals('id', this.blogId)]),
            );

            const blogs = await this.blogRepository.search(
                criteria,
                Context.api,
            );

            if (blogs.length > 0) {
                this.isSlugDuplicated = true;
                this.addBlogError({
                    property: 'slug',
                    message: this.$tc(
                        'sw-cms.detail.notification.messageSlugDuplicated',
                    ),
                    code: BLOG.BLOG_SLUG_DUPLICATE_ERROR_CODE,
                });
        ...
        cmsPageIsValidV640() {
            if (
                (this.isSystemDefaultLanguage && !this.page.name) ||
                !this.page.type
            ) {
                this.pageConfigOpen();
            ...
            const { foundEmptyRequiredField } = this.getSlotValidations(
                this.page.sections,
            );
            if (foundEmptyRequiredField.length > 0) {
                const warningMessage = this.$tc(
                    'sw-cms.detail.notification.messageMissingBlockFields',
                );
                this.createNotificationError({
                    message: warningMessage,
                });
        ...
        getLocaleLanguage() {
            return this.localeRepository
                .get(Context.api.language.localeId, Context.api)
                .then((result) => {
