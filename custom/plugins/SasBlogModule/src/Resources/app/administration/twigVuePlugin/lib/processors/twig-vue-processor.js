    ...
    processors: {
        'twig-vue': {
            ...
            preprocess(text, filename) {
                 ...
                 */
                const parsedText = text.startsWith(templateTagBefore)
                    ? text
                    : twigPreParser(text);

                return [{ text: parsedText, filename: filename }];
            },
            ...
            postprocess(messages, filename) {
                ...
                const finalMessages =
                    eslintPluginVueProcessor.postprocess(messages);

                // filter errors for fake <bloc> elements
                const finalMessagesFiltered = finalMessages.filter(
                    (message) =>
                        !(
                            message.ruleId === 'vue/html-self-closing' &&
                            message.message.includes('<bloc>')
                        )
                );
