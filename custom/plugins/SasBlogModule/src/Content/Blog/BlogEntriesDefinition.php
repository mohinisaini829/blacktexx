...
class BlogEntriesDefinition extends EntityDefinition
{
    ...
    protected function defineFields(): FieldCollection
    {
        return new FieldCollection([
            ...
            (new TranslatedField('metaTitle'))->addFlags(new ApiAware(), new SearchRanking(SearchRanking::MIDDLE_SEARCH_RANKING)),
            (new TranslatedField('metaDescription'))->addFlags(new ApiAware(), new SearchRanking(SearchRanking::MIDDLE_SEARCH_RANKING)),
            (new TranslatedField('metaKeywords'))->addFlags(new ApiAware(), new SearchRanking(SearchRanking::MIDDLE_SEARCH_RANKING)),
            ...
            (new OneToOneAssociationField('seoCategory', 'seoCategory', 'id', BlogCategoryDefinition::class))->addFlags(new ApiAware(), new Runtime()),

            (new ManyToManyAssociationField('blogCategories', BlogCategoryDefinition::class, BlogCategoryMappingDefinition::class, 'sas_blog_entries_id', 'sas_blog_category_id'))->addFlags(new CascadeDelete(), new ApiAware(), new SearchRanking(0.5)),
            (new ManyToManyAssociationField('assignedProducts', ProductDefinition::class, BlogProductMappingDefinition::class, 'sas_blog_entries_id', 'product_id'))->addFlags(new CascadeDelete(), new ApiAware()),
            (new ManyToOneAssociationField('blogAuthor', 'author_id', BlogAuthorDefinition::class, 'id', false))->addFlags(new ApiAware(), new SearchRanking(0.5)),
            (new OneToOneAssociationField('cmsPage', 'cms_page_id', 'id', CmsPageDefinition::class, false))->addFlags(new ApiAware()),
            (new ManyToManyAssociationField('tags', SasTagDefinition::class, BlogTagDefinition::class, 'sas_blog_id', 'sas_tag_id'))->addFlags(new CascadeDelete(), new ApiAware()),
        ]);
    }
}
