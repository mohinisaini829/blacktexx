...
class SasBlogModule extends Plugin
{
    ...
    public function uninstall(UninstallContext $context): void
    {
        ...
        $connection->executeStatement('DROP TABLE IF EXISTS `sas_blog_product`');
        ...
        $connection->executeStatement('DROP TABLE IF EXISTS `sas_blog_tag`');
        ...
        $criteria->addFilter(new EqualsAnyFilter('type', ['blog-detail', 'blog-listing', 'blog-assignment']));
    ...
    private function deleteSeoUrlTemplate(Context $context): void
    {
        $criteria = new Criteria();
        $criteria->addFilter(
            new EqualsAnyFilter('entityName', [BlogEntriesDefinition::ENTITY_NAME, BlogCategoryDefinition::ENTITY_NAME])
        );
    ...
    private function getBlogArticlesIds(): array
    {
        ...
        if (!$connection->createSchemaManager()->tablesExist([BlogEntriesDefinition::ENTITY_NAME])) {
            return [];
        }
