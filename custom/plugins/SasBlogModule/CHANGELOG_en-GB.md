# 4.0.6
- Fixed blog listing cache issue

# 4.0.5
- Fixed issue with Blog RSS feed
- Fixed issue with Blog detail page

# 4.0.4
- Fixed issue related to blog detail canonical URL generation.

# 4.0.3
- Fixed issue that custom fields were not displayed in the blog detail page.

# 4.0.2
- Fixed issue that sidebar was invisible in the blog detail page after switching languages.

# 4.0.1
- Fixed issue with uninstalled plugin

# 4.0.0
- Compatibility with Shopware 6.7

# 3.0.25
- Updated SEO URL generation for blogs that have no Sales Channel assigned.

# 3.0.24
- Added `blog.indexer` and `blog.category.indexer` to the Settings->Caches & indexes

# 3.0.23
- Compatibility with Shopware 6.6.x old version

# 3.0.22
- Fixed the issue that the blog slug could not work as expected.

# 3.0.21
- Added more twig blocks to better customize the blog detail page
- Put only category ids in the invalidation cache

# 3.0.20
- Centered blog detail post at Storefront

# 3.0.19
- Remove inherited flag from `cms_page` of the blog entries.

# 3.0.18
- Fixed the pagination issue on the blog listing page in the storefront when accessed via the blog category breadcrumb.

# 3.0.17
- Improved SEO canonical URL for blog detail page

# 3.0.16
- Updated some improvements

# 3.0.15
- Updated some improvements

# 3.0.14
- Fixed issue that the blog detail did not show the blocks section

# 3.0.13
- Added tag management for blog.

# 3.0.12
- Added search module for blog entries in the administration
- Removed special characters from the blog entry's slug
- Added Blog custom field sets.
- Fix Blog Seo URL generation when the blog entry is created in a specific sales channel.

# 3.0.11
- Fixed the bug that the blog author's description is not displayed with HTML format.
- Added `tags` association to the blog entry.
- Fixed the bug that could not create a new product detail and product listing page after installing the plugin `CbaxModulManufacturers`

# 3.0.10
- Changed route name from `sas.frontend.blog.search` to `frontend.sas.blog.search` to be compatible with Shopware pattern.

# 3.0.9
- Fixed the bug that the blog entry is able to view even if it is not published yet.
- Updated Blog's jsonld schema to display SEO URL of the blog entry's author.
- Now you can add tags for each blog entry.
- New tab `Blog Assignment` in the product detail page to assign blog entries to the product.
- Now you have 2 options to show the blog entries on the product detail page: Assign specific blog or assign blog by tags.
- Added a new config element to `cms-element-blog-assignment` to decide whether to show the blog entries in the order or randomly.

# 3.0.7
- Introduced the Blog Category Breadcrumb. 

# 3.0.6
- Implemented functionality to open the Blog search result when entering a search term, provided the Search result tab configuration is set to Blog.
- Added the display of the Blog Author name for the panel filter.

# 3.0.5
- Die Slug-Validierung wurde aktualisiert.

# 3.0.4
- Added block to the blog detail page to be able to add custom content for the rich snippet

# 3.0.3
- Updated the ld+json schema for blog entry to use author as person.

# 3.0.2
- Allow to edit name of the blog category in multiple languages

# 3.0.1
- Fixed issue related to the blog category
- Fixed the error that was shown with the console log from the storefront

# 3.0.0
- Compatibility with Shopware 6.6

# 2.11.9
- Fixed issue that display the old slug in the blog detail page.

# 2.11.8
- Fixed issue related to the unique slug constraint. This issue just happened with existing blogs that have the same slug
- Fixed issue that changed Blog Detail CMS but not invalidating the cache

# 2.11.7
- Fixed error when creating a new blog in different languages but same slug

# 2.11.6
- Slug is now editable in the blog entry
- Updated ld+json schema for blog entry
- Added meta keywords to the blog entry
- Added new plugin configuration to prioritize the blog search result's tab

# 2.11.5
- Should have the capability to toggle between two tabs (product and blog) within the search results.
- The Blog Search Box functionality should be automatically disabled if it has been deactivated in the plugin configuration.

# 2.11.4
- Automatically open the sidebar when creating a new blog entry
- Fix RSS feed to up-to-date with the latest blog SEO URL
- Fix error when displaying Blog RSS feed with special characters

# 2.11.3
- Fix error when displaying Blog RSS feed

# 2.11.2
- Fix error when migrating `Migration1609140710AddCmsPdpLayout` since 2.11.0

# 2.11.1
- Update Blog Slider plugin

# 2.11.0
- Added new feature to show specific blog posts on a product detail page.
