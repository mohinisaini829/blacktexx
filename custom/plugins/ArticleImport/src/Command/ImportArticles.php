...
#[AsCommand(
    ...
    description: 'Import Articles/Products from a file'
)]
class ImportArticles extends Command
{
    ...
    {
        die;
        $csvPath = __DIR__ . '/csv/HAKRO-Data-2025-imported-columns-2.csv';
        ...
        while (($row = fgetcsv($handle)) !== false) {
            ...
            /* Parent Product */
            $childProId       =   $this->importHelper->checkIfProductExist($data['Article Number']);
            if (empty($childProId)){

/*                if ($rowCount >= 200){
                    die("200 product created");
                }*/

                $this->processRow($data, $parentProIds[$masterSku]);
                $rowCount++;
                $output->writeln("Product created/updated {$data['3 Article Name']}");
            } else {
                $output->writeln("Product already created {$data['3 Article Name']}");
            }
        }
        fclose($handle);
    ...
    private function createChildProduct($childProData, $parentProductId){
        ...
        $masterColorSize    =       $childProData['6 Master Field'];
        ...
        try {
            ...
            echo "Unable to create variant for {$childProData['Article Number']}, error is {$e->getMessage()} ";
        }
    ...
    private function createProduct($data, $parentProId = '', $options = [], $colorOptId = '') {
        ...
        $ifProExist       =   $this->importHelper->checkIfProductExist($data['Article Number']);
        $variantUpData    =   [];
        if ($ifProExist) {
            ...
            try {
                ...
                $proData            =   [
                                            'id'    => $productId,
                                            'stock' => 10,
                                            'name'  => $mainDeData['ERP Article Name'],
                                            'isActive'  => $isActive,
                                            "productNumber" => $data['Article Number'],
                                            'description'   => $mainDeData['Article Description'],
                                            'taxId'         => $taxId,
                                            'maxPurchase'  => 10000,
                                            'visibilities'  => [
                                                [
                ...
                if (!empty($parentProId)) {
                    ...
                    $customFields[$customFieldPrefix.'gtin']         =  $data['EAN/GTIN'];
                    $customFields[$customFieldPrefix.'gender']      =  $data['12 Sex'];
                    ...
                    $customFields[$customFieldPrefix.'iteminbox']   =  $data['20 Pieces per Carton'];
                    $customFields[$customFieldPrefix.'prosheet']    =  $proSheet;
                    $customFields[$customFieldPrefix.'supgln']      =  $data['22 Supplier Article Number'];
                    $customFields[$customFieldPrefix.'washtemp']    =  $data['16 Washing Temperature'];
                    //$customFields[$customFieldPrefix.'modelcode']   =  $data['ModelCode'];
                    $customFields[$customFieldPrefix.'colorcode']   =  $data['5. Color Code'];
                    $customFields[$customFieldPrefix.'country']   =  $data['7 Country of Origin'];
                    $customFields[$customFieldPrefix.'material']   =  $data['24 Material Composition'];
                ...
                foreach ($tierPriceQty as $tierPriceQtyKey => $tierPriceQtyValue) {
                    $priceKey   =   "Price ".$tierPriceQtyValue;
