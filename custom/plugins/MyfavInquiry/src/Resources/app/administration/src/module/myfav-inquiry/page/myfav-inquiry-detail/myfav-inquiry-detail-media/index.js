...
Component.register('myfav-inquiry-detail-media', {
  ...
  methods: {
    ...
    setupDownloadListener() {
      ...
      $(document).on('click', '.myfav-inquiry-detail-media .media-thumbnail', (event) => {
        // Prevent download if another download is in progress
        if (this.isDownloading) return;

        // Disable all other thumbnails during the download
        $('.media-thumbnail').css('pointer-events', 'none');
        ...
        setTimeout(() => {
          ...
          $('.media-thumbnail').css('pointer-events', 'auto'); // Re-enable all thumbnails
