...
use Shopware\Core\Content\Media\File\DownloadResponseGenerator, Shopware\Core\Content\Media\File\FileFetcher, Shopware\Core\Content\Media\File\FileInfoHelper, Shopware\Core\Content\Media\File\FileLoader, Shopware\Core\Content\Media\File\FileNameProvider, Shopware\Core\Content\Media\File\FileNameValidator, Shopware\Core\Content\Media\File\FileService, Shopware\Core\Content\Media\File\FileUrlValidator, Shopware\Core\Content\Media\File\FileUrlValidatorInterface, Shopware\Core\Content\Media\File\MediaFile, Shopware\Core\Content\Media\File\WindowsStyleFileNameProvider;
...
class FileSaver extends \Shopware\Core\Content\Media\File\FileSaver
{
     ...
     */
    public function __construct(private readonly EntityRepository $mediaRepository, private readonly FilesystemOperator $filesystemPublic, private readonly FilesystemOperator $filesystemPrivate, private readonly ThumbnailService $thumbnailService, private readonly MetadataLoader $metadataLoader, private readonly TypeDetector $typeDetector, private readonly MessageBusInterface $messageBus, private readonly EventDispatcherInterface $eventDispatcher, private readonly SqlMediaLocationBuilder $locationBuilder, private readonly AbstractMediaPathStrategy $mediaPathStrategy, private readonly array $allowedExtensions, private readonly array $privateAllowedExtensions, private readonly bool $remoteThumbnailsEnable = false)
    {
    ...
    public function persistFileToMedia(MediaFile $mediaFile, string $destination, string $mediaId, Context $context): void
    {
        ...
        $this->ensureFileNameIsUnique($currentMedia, $destination, $mediaFile->getFileExtension(), $context);
        ...
        $media = $this->updateMediaEntity($mediaFile, $destination, $currentMedia, $metaData, $mediaType, $context);
    ...
    public function renameMedia(string $mediaId, string $destination, Context $context): void
    {
        ...
        $this->ensureFileNameIsUnique($currentMedia, $destination, $fileExtension, $context);
    ...
    private function doRenameMedia(MediaEntity $media, string $destination, Context $context): void
    {
        ...
        try {
            $renamedFiles = $this->renameFile($media->getPath(), $path, $this->getFileSystem($media));
        } catch (\Exception) {
        ...
        $updateData = ['id' => $media->getId(), 'fileName' => $destination, 'path' => $path];
        if ($this->remoteThumbnailsEnable === false) {
            ...
            if (!empty($thumbnails)) {
                foreach ($thumbnails as $thumbnailId => $thumbnailPath) {
                    $event->thumbnail(mediaId: $media->getId(), thumbnailId: $thumbnailId, path: $thumbnailPath);
                }
     ...
     */
    private function renameThumbnail(MediaThumbnailEntity $thumbnail, MediaEntity $currentMedia, string $destination): array
    {
        return [];
    }
     ...
     */
    private function updateMediaEntity(MediaFile $mediaFile, string $destination, MediaEntity $media, ?array $metadata, MediaType $mediaType, Context $context): MediaEntity
    {
        $data = ['id' => $media->getId(), 'userId' => $context->getSource() instanceof AdminApiSource ? $context->getSource()->getUserId() : null, 'mimeType' => $mediaFile->getMimeType(), 'fileExtension' => $mediaFile->getFileExtension(), 'fileSize' => $mediaFile->getFileSize(), 'fileName' => $destination, 'metaData' => $metadata, 'mediaTypeRaw' => serialize($mediaType)];
    ...
    private function findMediaById(string $mediaId, Context $context): MediaEntity
    {
        ...
        $criteria->addAssociation('mediaFolder');
        $currentMedia = $this->mediaRepository->search($criteria, $context)->getEntities()->get($mediaId);
     ...
     * @throws MediaException
     */
    private function ensureFileNameIsUnique(MediaEntity $currentMedia, string $destination, string $fileExtension, Context $context): void
    {
        $mediaWithRelatedFileName = $this->searchRelatedMediaByFileName($currentMedia, $destination, $fileExtension, $context);
        foreach ($mediaWithRelatedFileName as $media) {
            if (!$media->hasFile() || $destination !== $media->getFileName() || $media->isPrivate() !== $currentMedia->isPrivate()) {
                continue;
            }
            throw MediaException::duplicatedMediaFileName($destination, $fileExtension);
        }
    }
    private function searchRelatedMediaByFileName(MediaEntity $media, string $destination, string $fileExtension, Context $context): MediaCollection
    {
        $criteria = new Criteria();
        $criteria->addFilter(new MultiFilter(MultiFilter::CONNECTION_AND, [new EqualsFilter('fileName', $destination), new EqualsFilter('fileExtension', $fileExtension), new NotFilter(NotFilter::CONNECTION_AND, [new EqualsFilter('id', $media->getId())])]));
