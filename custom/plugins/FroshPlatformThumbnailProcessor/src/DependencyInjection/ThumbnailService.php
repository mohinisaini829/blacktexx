...
class ThumbnailService extends \Shopware\Core\Content\Media\Thumbnail\ThumbnailService
{
     ...
     */
    public function __construct(private readonly EntityRepository $thumbnailRepository, private readonly FilesystemOperator $filesystemPublic, private readonly FilesystemOperator $filesystemPrivate, private readonly EntityRepository $mediaFolderRepository, private readonly EventDispatcherInterface $dispatcher, private readonly EntityIndexer $indexer, private readonly ThumbnailSizeCalculator $thumbnailSizeCalculator, private readonly Connection $connection, private readonly bool $remoteThumbnailsEnable = false)
    {
    }
    ...
    private function generateAndSave(MediaEntity $media, MediaFolderConfigurationEntity $config, Context $context, ?MediaThumbnailSizeCollection $sizes): array
    {
        ...
        foreach ($sizes as $size) {
            ...
            $records[] = ['id' => $id, 'mediaId' => $media->getId(), 'width' => $size->getWidth(), 'height' => $size->getHeight()];
        }
     ...
     * @return ImageSize
     */
    private function calculateThumbnailSize(array $imageSize, MediaThumbnailSizeEntity $preferredThumbnailSize, MediaFolderConfigurationEntity $config): array
    {
        if (!$config->getKeepAspectRatio()) {
            return $this->thumbnailSizeCalculator->determineValidSize($imageSize, $preferredThumbnailSize->getWidth(), $preferredThumbnailSize->getHeight());
        }
    ...
    private function createNewImage(\GdImage $mediaImage, MediaType $type, array $originalImageSize, array $thumbnailSize): \GdImage
    {
        ...
        imagecopyresampled($thumbnail, $mediaImage, 0, 0, 0, 0, $thumbnailSize['width'], $thumbnailSize['height'], $originalImageSize['width'], $originalImageSize['height']);
        return $thumbnail;
    }
    ...
    {
        return $media->getMediaType() instanceof ImageType && !$media->getMediaType()->is(ImageType::VECTOR_GRAPHIC) && !$media->getMediaType()->is(ImageType::ANIMATED) && !$media->getMediaType()->is(ImageType::ICON);
    }
    ...
    {
        return $thumbnail->getWidth() === $thumbnailSize->getWidth() && $thumbnail->getHeight() === $thumbnailSize->getHeight();
    }
}